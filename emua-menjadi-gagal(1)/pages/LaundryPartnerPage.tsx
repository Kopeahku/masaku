...
const CheckboxField: React.FC<{ label: string, id: string, name: string, checked: boolean, onChange: (e: React.ChangeEvent<HTMLInputElement>) => void }> = ({ label, id, name, checked, onChange }) => (
    <label htmlFor={id} className="flex items-center gap-2 p-2 border rounded-md bg-white dark:bg-slate-800 dark:border-slate-600 cursor-pointer">
        <input type="checkbox" id={id} name={name} checked={checked} onChange={onChange} className="h-4 w-4 rounded text-primary focus:ring-primary" />

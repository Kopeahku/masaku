...
import { getFinancialAdvice, isAiAvailable } from '../services/geminiService.ts';
...
const AiDisabledCard: React.FC = () => (
    <div className="bg-surface dark:bg-dark-surface rounded-xl shadow-md p-4 flex flex-col h-full border border-slate-200 dark:border-slate-700 items-center justify-center text-center">
        <InformationCircleIcon className="w-10 h-10 text-slate-400 dark:text-slate-500 mb-2" />
        <h3 className="text-base font-bold text-text-primary dark:text-dark-text-primary">Fitur AI Dinonaktifkan</h3>
        <p className="text-sm text-text-secondary dark:text-dark-text-secondary mt-1">
            Untuk menggunakan Penasihat Keuangan AI, harap konfigurasikan Gemini API Key pada environment variabel <strong>VITE_API_KEY</strong> di platform hosting Anda.
        </p>
    </div>
);
...
const MemberDashboard: React.FC<MemberDashboardProps> = ({ user, transactions, theme, setActivePage, featureVisibility, onShowComingSoon, getTransactionData, setInitialWithdrawalTab }) => {
  ...
  // Filter out AI-dependent features if the API key is not available
  const visibleFeatures = dashboardFeatures.filter(feature => {
      const isAiFeature = feature.id === 'dreamWeaver';
      return isAiAvailable || !isAiFeature;
  });
